...
use common_game::components::resource::{
    BasicResource, BasicResourceType, Carbon, ComplexResourceRequest, ComplexResourceType,
    GenericResource,
};
        ...
        PlanetToExplorer::GenerateResourceResponse { resource } => match resource {
            Some(resource) => {
                if resource.get_type() == asked_resource {
                    println!(
                        "Explorer {} received requested resource {:?}",
                        expl_id, asked_resource
                    );
                    Ok(Some(resource))
                } else {
                    Err(format!(
                        "Received a different resource from requested type: {:?}, received: {:?}",
                        asked_resource,
                        resource.get_type()
                    ))
                }
            ...
            None => {
                println!(
                    "Explorer {} DID NOT receive requested resource {:?}",
                    expl_id, resource
                );
                Ok(None)
            }
...
pub fn get_combination_list(
    sender_expl_to_planet: &Sender<ExplorerToPlanet>,
    receiver_planet_to_expl: &Receiver<PlanetToExplorer>,
    expl_id: ID,
) -> Result<HashSet<ComplexResourceType>, String> {
    sender_expl_to_planet
...
pub fn get_supported_resources(
    sender_expl_to_planet: &Sender<ExplorerToPlanet>,
    receiver_planet_to_expl: &Receiver<PlanetToExplorer>,
    expl_id: ID,
) -> Result<HashSet<BasicResourceType>, String> {
    sender_expl_to_planet
...
pub fn get_available_energy_cells(
    sender_expl_to_planet: &Sender<ExplorerToPlanet>,
    receiver_planet_to_expl: &Receiver<PlanetToExplorer>,
    expl_id: ID,
) -> Result<ID, String> {
...
pub fn combine_resources(
    ...
    match resp {
        PlanetToExplorer::CombineResourceResponse { complex_response } => match complex_response {
            Ok(_) => Err(
