...
use crate::tests::explorer::{ask_for_resource, combine_resources, get_available_energy_cells, get_combination_list, get_supported_resources};
use crate::tests::handle_planet::{get_internal_state, kill_planet, start_planet_ai, stop_planet_ai};
use crate::tests::orchestrator::{incoming_explorer, send_asteroid, send_multiple_sunrays, send_sunray};
use crate::tests::utils::{get_forge, init_logger, init_test_planet, start_planet_thread};

#[test]
fn test_sunray() {
    init_logger();
    let (rusteroids, sx_orch_to_planet, rx_planet_to_orch, _sx_expl_to_planet, _sx_planet_to_expl, _rx_planet_to_expl) = init_test_planet();
...
fn test_extraction() {
    init_logger();
    let (rusteroids, sx_orch_to_planet, rx_planet_to_orch, sx_expl_to_planet, sx_planet_to_expl, rx_planet_to_expl) = init_test_planet();
    start_planet_thread(rusteroids);
    ...
    incoming_explorer(&sx_orch_to_planet, &rx_planet_to_orch, explorer_id, sx_planet_to_expl);

    for _ in 0..num_resource {
        ask_for_resource(&sx_expl_to_planet, &rx_planet_to_expl, asked_resource, explorer_id).unwrap();
...
fn test_combine_resources() {
    ...
    let c1 = ask_for_resource(&sx_expl_to_planet, &rx_planet_to_expl, BasicResourceType::Carbon, explorer_id).unwrap().unwrap().to_carbon().unwrap();
    let c2 = ask_for_resource(&sx_expl_to_planet, &rx_planet_to_expl, BasicResourceType::Carbon, explorer_id).unwrap().unwrap().to_carbon().unwrap();
...
fn test_planet_stop() {
    ...
    let forge = get_forge();
    let res = send_sunray(&sx_orch_to_planet, &rx_planet_to_orch, forge.generate_sunray());
    match res {
        Err(e) => {
