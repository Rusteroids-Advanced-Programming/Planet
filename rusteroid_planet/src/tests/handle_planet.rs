...
pub fn stop_planet_ai(
    ...
    match resp {
        ...
        _ => Err(format!(
            "Received another msg {:?} instead of the stop planet",
            resp
        )),
    }
}
...
pub fn kill_planet(
    ...
    match resp {
        PlanetToOrchestrator::KillPlanetResult { planet_id: _ } => Ok(()),
...
pub fn get_internal_state(
    sender_orch_to_planet: &Sender<OrchestratorToPlanet>,
    receiver_planet_to_orch: &Receiver<PlanetToOrchestrator>,
) -> Result<DummyPlanetState, String> {
    let resp = send_message_and_wait_response(
        sender_orch_to_planet,
        receiver_planet_to_orch,
    ...
    match resp {
        ...
        _ => Err(format!(
            "Received another msg {:?} instead of the internal state ack",
            resp
        )),
    }
}
